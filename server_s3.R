...
pacman::p_load(shiny, ggplot2, plotly, reshape, rgdal, RColorBrewer, ggdendro, dendextend, RCurl)
...
shinyServer(function(input, output, session){
  ...
  Sys.setenv("AWS_ACCESS_KEY_ID" = "AKIAJBOMPSJ7FPIHIXZA","AWS_SECRET_ACCESS_KEY" = "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", "AWS_DEFAULT_REGION" = "us-east-1")
  ...
  #### reactive ctryAdmLevels ####
    ...
    ctryAdmLevels <- reactive({
      message(paste0("here: ctryAdmLevels"))
      ...
      usercsvobj <-get_object(paste0("s3://shiny-nightlights/", getCtryNlDataFnamePath(input$countries)))
      csvcharobj <- rawToChar(usercsvobj)
      con <- textConnection(csvcharobj)
      temp <- read.csv(con, nrows = 1, header = T)
      close(con)
    ...
    ctryAdmLevelNames <- reactive({
      message(paste0("here: ctryAdmLevelNames"))
      ...
      usercsvobj <-get_object(paste0("s3://shiny-nightlights/", getCtryNlDataFnamePath(countries)))
      csvcharobj <- rawToChar(usercsvobj)
      con <- textConnection(csvcharobj)
      hdr <- read.csv(con, nrows = 1, header = T)
      close(con)
      ...
      csvcharobj <- rawToChar(usercsvobj)
      con <- textConnection(csvcharobj)
      data <- read.csv(con, colClasses = colClasses, header =T)
      close(con)
    ...
    ctryNlData <- reactive({
      message(paste0("here: ctryNlData"))
      input$btnGo
      ...
      csvcharobj <- rawToChar(usercsvobj)
      con <- textConnection(csvcharobj)
      ctryData <- read.csv(con, header =T)
      close(con)
      }
...
#          temp <- fread(getCtryNlDataFnamePath(ctryCode))
      usercsvobj <-get_object(paste0("s3://shiny-nightlights/", getCtryNlDataFnamePath(ctryCode)))
      csvcharobj <- rawToChar(usercsvobj)
      con <- textConnection(csvcharobj)
      temp <- read.csv(con, header =T)
      close(con)
    ...
    ctryNlDataMelted <- reactive({
      message(paste0("here: ctryNlDataMelted"))
    ...
    values <- reactiveValues(
      a=message(paste0("here: lastUpdated")),
      lastUpdated = NULL
    )
    ...
    output$intraCountry <- renderUI({
      ...
      message("here: renderUI intracountry")
      countries <- input$countries
    ...
    observe({
      message(paste0("here: observe selectAdms"))
      ...
      message(paste0("lastupdated:",values$lastUpdated))
      
      message(paste0("x:",x))
      message(paste0("lvlnum:",lvlNum))
      ...
      for (lvlIdx in 2:length(ctryAdmLevels))
      {
        ...
        if (lvlIdx < lvlNum)
        {
          message(paste0("lvlIdx:",lvlIdx,"lvlNum:",lvlNum))
          ...
          #message(paste0("lvlSelect:",lvlSelect))
          
          #message(paste0("lvlselect: ", lvlSelect))
          #message(paste0("top10: ", top10))
  ...
  #})
    ...
    output$sliderNlYearMonthRange <- renderUI({
      message(paste0("here: sliderNlYearMonthRange"))
    ...
    output$sliderNlYearMonth <- renderUI({
      message(paste0("here: sliderNlYearMonth"))
    ...
    ####reactive hcluster####
    hCluster <- reactive({
      message(paste0("here: reactive hCluster"))
      input$btnGo
    ...
    output$plotHCluster <- renderPlot({
      message(paste0("here: plotHCluster"))
    ...
    output$plotPointsCluster <- renderPlotly({
      message(paste0("here: plotPointsCluster"))
    ...
    output$mapHCluster <- renderLeaflet({
      message(paste0("here: draw mapHCluster"))
      ...
      isolate({      
        ...
        message("drawing leaflet cluster")
        ...
        map <- leaflet(data=ctryPoly0) %>%
          ...
          addTiles %>%
          addWMSTiles(layerId="nlRaster", baseUrl = "https://localhost/cgi-bin/mapserv?map=nightlights_wms.map", layers = "nightlights_201204", options = WMSTileOptions(format = "image/png", transparent = TRUE, opacity=1)) %>%
    ...
    output$plotTSDecomposed <- renderPlot({
      message(paste0("here: plotTSDecomposed"))
      input$btnGo
    ...
    output$plotYearly <- renderPlotly({
      message(paste0("here: renderPlotYearly"))
      input$btnGo
      ...
      isolate({      
        ...
        for (i in x)
          ...
          message(paste0("x", x))
          message(paste0("admlvlnums:", admLvlNums))
          ...
          message(paste0("admLevel:", admLevel))
          ...
          message(paste0("ctrydata nrow:", nrow(ctryData)))
    ...
    output$plotNightLights <- renderPlotly({
      message(paste0("here: renderPlot"))
      input$btnGo
...
#       message(paste0("here: observe admLevel 2 update map"))
...
#     })
    ...
    output$map <- renderLeaflet({
      message(paste0("here: draw leaflet map"))
      # Use leaflet() here, and only include aspects of the map that
      # won't need to change dynamically (at least, not unless the
      # entire map is being torn down and recreated).
...
#       }
      ...
      map <- leaflet(data=ctryPoly0) %>%
        ...
        for (iterAdmLevel in 2:lyrNum)
        {
          ...
          {
              message("process selected ", iterPoly)
              map <- map %>% addPolygons(
